package com.tabbara.mohammad.uielements.yahoo.rangeseekbar;
/*
...
*/
        ...
        import com.tabbara.mohammad.uielements.R;
        import com.tabbara.mohammad.uielements.yahoo.PixelUtil;
...
public class RangeSeekBar<T extends Number> extends AppCompatImageView {
    ...
    public static final Integer DEFAULT_MAXIMUM = 30;
...
//        }
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        listener.onRangeSeekBarValuesChanged(this, normalizedMinValue, normalizedMaxValue);
                    }
                }
                break;
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        c2.add(Calendar.DATE, (int) (-30 +30*normalizedMinValue));
        String minDate = dateFormat.format(c2.getTime());
        String minLabel = minDate;
        String maxDate = dateFormat.format(c1.getTime());
        String maxLabel = maxDate;
        String arrowLabel = getContext().getString(R.string.arrow);
        float arrowLabelSize = paint.measureText(arrowLabel);
//        float minMaxLabelSize = Math.max(paint.measureText(minLabel), paint.measureText(maxLabel));
        float minLabelSize = paint.measureText(minLabel);
        float maxLabelSize = paint.measureText(maxLabel);
        float minMaxHeight = thumbHalfHeight + mTextSize / 3;
//        String minText = ;
//        String maxText = String.valueOf(getSelectedMaxValue());
        canvas.drawText(minLabel, 0, 3 * thumbHalfHeight + minMaxHeight, paint);
        canvas.drawText(maxLabel, getWidth() - maxLabelSize, 3 * thumbHalfHeight + minMaxHeight, paint);
        canvas.drawText(arrowLabel, (getWidth() - arrowLabelSize)/2 , 3 * thumbHalfHeight + minMaxHeight, paint);
        padding = INITIAL_PADDING + thumbHalfWidth;

        // draw seek bar background line
//        Drawable d = getResources().getDrawable(R.mipmap.slider_background);
//        d.setBounds(left, top, right, bottom);

        mRect.left = padding;
        mRect.right = getWidth() - padding;
//        canvas.drawRect(mRect, paint);
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.slider_background);
